  ...
  return (
    <nav className="navbar">
      ...
      <div className="navbar__container">
        ...
        {/* Menu Desktop — à droite grâce à justify-content: space-between */}
        <div className="navbar__desktop-menu">
          ...
          {/* Loupe */}
          <button className="navbar__search-btn" onClick={handleSearchToggle} aria-label="Rechercher">
            <svg className="navbar__search-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round">
          ...
          {!isLoggedIn
            ...
            : <button className="navbar__btn-account">Mon compte</button>
          }
        </div>

        {/* Hamburger Mobile */}
        <button className="navbar__hamburger" onClick={() => setIsMenuOpen(!isMenuOpen)} aria-label="Menu">
          {isMenuOpen ? '✕' : '☰'}
        </button>
      </div>
      ...
      {showSearch && (
        <div className="navbar__search-bar" ref={searchRef}>
          <div className="navbar__search-wrapper">
            <svg className="navbar__search-bar-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              <circle cx="11" cy="11" r="7" />
              <line x1="16.5" y1="16.5" x2="22" y2="22" />
            </svg>
            <input
                ...
                : searchResults.map((product) => (
                  <div key={product.id} onClick={() => handleProductClick(product.id)} className="navbar__search-result-item">
                    ...
                    <div className="navbar__result-info">
                      ...
                      <div className="navbar__result-desc">
                        {product.description?.replace(/<[^>]*>/g, ' ').substring(0, 60)}...
                      </div>
                      <div className="navbar__result-price">{product.price} €</div>
                    </div>
                  </div>
                ))
              }
